...
def handler(event, context):
    try:
        ...
        for i, s_cidr in enumerate(subnet_cidrs):
            sn = EC2.create_subnet(VpcId=vpc_id, CidrBlock=s_cidr)
            subnet_ids.append(sn["Subnet"]["SubnetId"])
            EC2.create_tags(Resources=[sn["Subnet"]["SubnetId"]],
